...
const obtenerArmasApi = async () => {
  try {
    ...
    console.error('Error al obtener armas desde el API:', error);
    throw error;
  }
};
const crearArmaEnAPI = async (formData, file) => {
  try {
    ...
    const armaEndpoint = 'https://cards.thenexusbattles2.cloud/api/consumible/';
    const response = await fetch(armaEndpoint, {
      method: 'POST',
      body: data,
      headers: {
    ...
    console.error('Error al crear el arma en el API:', error);
    throw error;
  }
};
const actualizarArmaEnAPI = async (formData, file) => {
  try {
    // Extraer los campos del formulario individualmente
    const id = formData.id; // Asegúrate de que tengas un campo 'id' para identificar el arma que se actualizará
    ...
    // Construir la URL del API (asegúrate de incluir el ID en la URL para identificar el arma específica)
    ...
    // Realizar la solicitud PATCH al API para actualizar el arma
    const response = await fetch(apiUrl, {
      method: 'PATCH',
      body: data,
      headers: headers,
...
// Función para cambiar el estado de arma en el modelo
const cambiarEstadoArmaEnModelo = async (armaId, nuevoEstado) => {
  try {
    ...
    // Realizar la solicitud PATCH a la API externa para cambiar el estado del arma en el modelo
    const response = await fetch(apiUrl, {
      method: 'PATCH',
      body: formData,
    });
    ...
    console.error('Error al cambiar el estado del arma en el modelo:', error);
    throw error;
  }
};
