...
const obtenerItemsApi = async () => {
  try {
    ...
    console.error('Error al obtener items desde el API:', error);
    throw error;
  }
};
const crearItemEnAPI = async (formData, file) => {
  try {
    ...
    console.error('Error al crear el item en el API:', error);
    throw error;
  }
};
const actualizarItemEnAPI = async (formData, file) => {
  try {
    // Extraer los campos del formulario individualmente
    const id = formData.id; // Asegúrate de que tengas un campo 'id' para identificar el item que se actualizará
    ...
    // Construir la URL del API (asegúrate de incluir el ID en la URL para identificar el item específico)
    ...
    // Realizar la solicitud PATCH al API para actualizar el item
    const response = await fetch(apiUrl, {
      method: 'PATCH',
      body: data,
      headers: headers,
...
// Función para cambiar el estado del item en el modelo
const cambiarEstadoItemEnModelo = async (itemId, nuevoEstado) => {
  try {
    ...
    // Realizar la solicitud PATCH a la API externa para cambiar el estado del item en el modelo
    const response = await fetch(apiUrl, {
      method: 'PATCH',
      body: formData,
    });
    ...
    console.error('Error al cambiar el estado del item en el modelo:', error);
    throw error;
  }
};
